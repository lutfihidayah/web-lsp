...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl">
    <div class="mb-6">
        <h2 class="font-bold text-gray-800 text-lg">Edit Informasi / Berita</h2>
        <p class="text-sm text-gray-400 mt-1">Perbarui konten: <span class="font-medium text-gray-600 truncate">{{ $informasi->judul }}</span></p>
    </div>
    ...
    <form action="{{ route('informasi.update', $informasi->id) }}" method="POST" class="space-y-5">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="kategori" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e] text-gray-700">
                    ...
                    <option value="{{ $kat }}" {{ old('kategori', $informasi->kategori) == $kat ? 'selected' : '' }}>{{ $kat }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Penulis</label>
                <input type="text" name="penulis" value="{{ old('penulis', $informasi->penulis) }}"
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e]">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="isi" rows="6" required
                class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e]">{{ old('isi', $informasi->isi) }}</textarea>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('informasi.index') }}" class="px-6 py-2.5 border border-gray-200 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 transition">
                Batal
            </a>
        </div>
    </form>
