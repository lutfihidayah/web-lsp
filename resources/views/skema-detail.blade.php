...
<nav class="bg-white shadow-sm px-8 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
        @auth
            <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('user.dashboard') }}" class="px-5 py-2 bg-[#1e3a6e] text-white rounded-lg text-sm font-medium hover:bg-[#16305c]">Dashboard</a>
        @else
            ...
            <div class="border-t border-gray-100 pt-8 flex flex-col sm:flex-row items-center gap-4">
                @auth
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('admin.skema') }}" class="w-full sm:w-auto text-center px-8 py-3.5 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition">
                            Kelola Skema di Admin
                        </a>
                    @else
                        <a href="{{ route('user.skema.show', $skema->id) }}" class="w-full sm:w-auto text-center px-8 py-3.5 bg-[#1e3a6e] text-white font-bold rounded-xl hover:bg-[#16305c] transition flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20">
