...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl">
    <div class="mb-6">
        <h2 class="font-bold text-gray-800 text-lg">Edit Pertanyaan Ujian</h2>
        <p class="text-sm text-gray-400 mt-1">Perbarui data pertanyaan untuk ujian skema sertifikasi</p>
    </div>
    ...
    <form action="{{ route('soal.update', $soal->id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="skema_id" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e] text-gray-700">
                ...
                <option value="{{ $sk->id }}" {{ old('skema_id', $soal->skema_id) == $sk->id ? 'selected' : '' }}>{{ $sk->nama }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <textarea name="pertanyaan" rows="4" required
                ...
                placeholder="Tuliskan teks pertanyaan di sini...">{{ old('pertanyaan', $soal->pertanyaan) }}</textarea>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="pilihan_a" value="{{ old('pilihan_a', $soal->pilihan_a) }}" required
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e]"
            ...
            <div>
                ...
                <input type="text" name="pilihan_b" value="{{ old('pilihan_b', $soal->pilihan_b) }}" required
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e]"
            ...
            <div>
                ...
                <input type="text" name="pilihan_c" value="{{ old('pilihan_c', $soal->pilihan_c) }}" required
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e]"
            ...
            <div>
                ...
                <input type="text" name="pilihan_d" value="{{ old('pilihan_d', $soal->pilihan_d) }}" required
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e3a6e]"
